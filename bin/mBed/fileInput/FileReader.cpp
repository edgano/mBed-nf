 ...
 */
bool FileReader::noEmptySequence(vector<Sequence> seqVector)
{
 ...
 */
int FileReader::seqInput(Alignment *alignPtr, bool append)
{
    int code;
    ...
    if (append)
    {
        ...
        code = readSeqs(alignPtr, numSeqsAlready + 1);
    }
    else
    {
        code = readSeqs(alignPtr, 1);  //  1 is the first seq to be read 
    }
 ...
 */
int FileReader::readSeqs(Alignment *alignPtr, int firstSeq)
{
    string line;
    string fileName;
    ...
    seqRangeVector = fileParser->getSeqRange(1, noSeqs);
    if (seqRangeVector.size()==0)
        return fileParser->getParseExitCode();
    // FIXME AW: noEmptySequence check should be done internally by FileParser instances
    if (noEmptySequence(seqRangeVector) == false)
    ...
    for (int i=0; i < (int)seqRangeVector.size(); i++) {
        ...
        // which important when no alignment (convert!) is done
...
#if 0
        if(alignPtr->checkAllNamesDifferent() == false) {  //nige moved
    ...
    else // profile2
    {
        ...
        if (! alignPtr->testUniqueNames(&seqVector)) {  //nige added
...
int FileReader::profileInput(Alignment *alignPtr)
{
    ...
    {                    
        ...
        code = readSeqs(alignPtr, profile1NumOfSeqs + 1); 
        if(code == OK)
        {
